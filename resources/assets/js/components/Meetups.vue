<template>
    <div>
                    ...
                    <nav aria-label="pagination">
                        <ul class="pagination">
                            ...
                            <li class="page-item" v-bind:class="{ active: currentPage === page }" v-for="page in pages">
                                <a class="page-link" v-on:click="getItems('api/meetups?page=' + page)">{{ page }}</a>
        ...
        <h2 class="text-center text-muted" v-else>
            There are no meetups at this time. How about <strong><a href="/events/create">submit</a></strong> one?
        </h2>
    </div>
</template>
